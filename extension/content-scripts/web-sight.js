// ============================================================
// Web-Sight v5 — Agentic Accessibility Browser Agent
// Fixes: infinite loop, message corruption, start/stop UX,
// navigation via background, new tab, go back, restart
// ============================================================

(function () {
  'use strict';

  if (window.__websight_v5) return;
  window.__websight_v5 = true;

  const SYSTEM_PROMPT = `You are Web-Sight, a voice-controlled AI browser agent for accessibility.

CRITICAL RULES:
1. NEVER get stuck in a loop. If a tool fails twice, stop and tell the user.
2. NEVER say "I can't" or "Please do this manually". You control the browser fully.
3. Keep final responses to 1 sentence maximum.
4. ALWAYS call get_page_context before clicking anything.

TOOLS:
- navigate_to(url): Go to URL in current tab
- open_new_tab(url): Open URL in a NEW tab
- go_back(): Go to previous page
- close_tab(): Close current tab
- get_page_context(): See all interactive elements numbered [0],[1],[2]...
- find_elements(description): Search for elements
- click_element(selector, description): Click an element
- type_text(selector, text): Type into an input (ALWAYS click the input first)
- press_key(key): Enter / Escape / Tab / ArrowDown / ArrowUp
- scroll_page(direction): up / down / top / bottom
- read_page(): Read page text
- select_option(selector, value): Choose from dropdown

TYPING RULE — VERY IMPORTANT:
To type in any search box:
  1. click_element on the input
  2. type_text with the text
  3. press_key "Enter"
NEVER call type_text more than once for the same field. If it fails, stop.

ORDINAL SELECTION ("first", "1st", "second", "2nd"):
- Call get_page_context
- "first" = index [0], "second" = [1], "third" = [2]
- Use the EXACT selector shown next to that index number

NAVIGATION:
- "open YouTube" → navigate_to("https://youtube.com")
- "open YouTube in new tab" → open_new_tab("https://youtube.com")
- "go back" → go_back()
- Always use https:// in URLs

LOOP PREVENTION:
- Max 2 retries for any single action
- If you see the same failing call 3 times: STOP immediately
- Never repeat type_text on the same field more than once

Only ask "confirm" for: buy, purchase, checkout, pay, place order.`;

  const HISTORY_KEY = 'websight_v5_history';
  ...
  // ─── Tools ────────────────────────────────────────────────
  const TOOLS = [
    mkfn('get_page_context', 'Get current URL, title, headings, and ALL interactive elements with index [0],[1],[2]. Call this before clicking anything.', {}),
    mkfn('find_elements', 'Find elements matching a description.', { description: { type: 'string' } }, ['description']),
    mkfn('click_element', 'Click an element by CSS selector.', { selector: { type: 'string' }, description: { type: 'string' } }, ['selector', 'description']),
    mkfn('type_text', 'Type text into focused input. ALWAYS click the input first.', { selector: { type: 'string' }, text: { type: 'string' }, clear_first: { type: 'boolean' } }, ['text']),
    ...
    mkfn('open_new_tab', 'Open URL in a NEW browser tab. Use when user says "new tab".', { url: { type: 'string' } }, ['url']),
    mkfn('close_tab', 'Close current browser tab.', {}, []),
    mkfn('go_back', 'Go back to previous page.', {}, []),
    mkfn('read_page', 'Read visible page text.', { selector: { type: 'string' } }, []),
    mkfn('select_option', 'Select from a <select> dropdown.', { selector: { type: 'string' }, value: { type: 'string' } }, ['selector', 'value']),
  ];
  ...
  async function runTask(command) {
    if (isRunning) { cancelTask = true; await wait(300); cancelTask = false; }
    ...
    try {
      while (steps < MAX_STEPS) {
        ...
        const resp = await ipc({ type: 'API_REQUEST', model: 'gpt-4o', messages, tools: TOOLS, tool_choice: 'auto', max_tokens: 600, temperature: 0.05 });
        ...
        if (choice.finish_reason === 'tool_calls' && choice.message?.tool_calls?.length) {
          // MUST push assistant message with tool_calls BEFORE pushing tool results
          ...
          for (const tc of choice.message.tool_calls) {
            ...
            if (recentCalls.filter(c => c === sig).length >= 3) {
              messages.push({ role: 'tool', content: JSON.stringify({ error: 'Loop detected, aborting.' }), tool_call_id: tc.id });
              reply = `I kept failing to ${tc.function.name.replace(/_/g,' ')} and stopped to avoid looping. Please try rephrasing.`;
              isRunning = false;
            ...
            // Always push tool result with matching tool_call_id
            messages.push({ role: 'tool', content: JSON.stringify(result), tool_call_id: tc.id });

            // Safe trim — only remove a message if it won't break assistant/tool pairs
            if (messages.length > 28 && messages[1]?.role === 'user') messages.splice(1, 1);
          }
  ...
  function toolLabel(name, args) {
    return { click_element:`Clicking: ${args.description||args.selector||''}`, type_text:`Typing: "${(args.text||'').slice(0,50)}"`, press_key:`Key: ${args.key}`, navigate_to:`Going to: ${args.url}`, open_new_tab:`New tab: ${args.url}`, close_tab:'Closing tab', go_back:'Going back', scroll_page:`Scrolling ${args.direction}`, find_elements:`Finding: ${args.description}`, get_page_context:'Reading page', read_page:'Reading content', select_option:`Selecting: ${args.value}` }[name] || name;
  }
  ...
  async function execTool(name, args) {
    switch (name) {
      ...
      case 'click_element': {
        ...
        await wait(150); el.click(); el.focus(); await wait(400);
      ...
      case 'type_text': {
        ...
        if (!el) {
          for (const inp of document.querySelectorAll('input[type="text"],input[type="search"],input[type="email"],input[type="url"],input:not([type]),textarea,[contenteditable="true"]')) {
            ...
            if (cs.display !== 'none' && cs.visibility !== 'hidden') { el = inp; break; }
          }
        }
        if (!el) return { success: false, error: 'No input found. Click the input first.' };
        const text = args.text || '';
        highlight(el); el.scrollIntoView({ behavior: 'smooth', block: 'center' }); el.focus(); await wait(80);
        if (el.isContentEditable) {
          ...
          if (args.clear_first !== false) {
            const ns = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value')?.set || Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, 'value')?.set;
        ...
        await wait(200);
        return { success: true, typed: text, actual: (el.isContentEditable ? el.textContent : el.value)?.slice(0, 60) };
      }
      ...
      case 'press_key': {
        ...
        if (args.key === 'Enter' && el.tagName === 'INPUT' && el.form) { try { el.form.requestSubmit ? el.form.requestSubmit() : el.form.submit(); } catch (e) {} }
        await wait(500);
        return { success: true };
      }
      ...
      case 'navigate_to': {
        ...
        history.push({ role: 'ai', type: 'action', text: `Navigating to ${url}`, time: ts() });
      ...
      case 'open_new_tab': {
        ...
        if (!url) return { success: false, error: 'No URL' };
        if (!/^https?:\/\//i.test(url)) url = 'https://' + url;
        addMsg('info', `Opening new tab: ${url}…`);
        const r = await ipc({ type: 'WEBSIGHT_OPEN_TAB', url });
        if (r?.success) return { success: true, url };
        const w = window.open(url, '_blank');
        return w ? { success: true, url } : { success: false, error: 'Popup blocked — try again' };
      }

      case 'close_tab': {
        const r = await ipc({ type: 'WEBSIGHT_CLOSE_TAB' });
        return r?.success ? { success: true } : { success: false, error: 'Failed to close tab' };
      }

      case 'go_back':
        window.history.back(); await wait(800);
        return { success: true };
      ...
      case 'read_page': {
        ...
        return { success: true, content: (el || document.body).innerText.trim().replace(/\s+/g, ' ').slice(0, 3000) };
      }
  ...
  function findEls(description) {
    ...
    document.querySelectorAll('a[href],button,input:not([type="hidden"]),select,textarea,[role="button"],[role="link"],[role="menuitem"],[role="tab"]').forEach((el, idx) => {
      ...
      const cs = window.getComputedStyle(el);
      if (cs.display === 'none' || cs.visibility === 'hidden') return;
      const s = sel(el); if (seen.has(s)) return;
      const d = descEl(el); if (!d) return;
  ...
  function findByText(text) {
    ...
    document.querySelectorAll('button,a,[role="button"],[role="link"]').forEach(el => {
      ...
      if (!t) return;
      let score = t === lower ? 100 : t.includes(lower) ? 80 : lower.includes(t) && t.length > 3 ? 60 : lower.split(/\s+/).filter(w => w.length > 2 && t.includes(w)).length * 15;
  ...
  function checkConfirm(text) {
    ...
    else { pendingConfirm = null; addMsg('system', 'Cancelled.'); }
    return true;
  }
  ...
  async function initPane() {
    ...
    style.textContent = `
      ...
      .ws5{display:flex;flex-direction:column;height:100%;overflow:hidden;font-family:'Syne',sans-serif;}
      .ws5-hdr{display:flex;align-items:center;gap:10px;padding:12px 14px 10px;flex-shrink:0;border-bottom:1px solid rgba(255,255,255,.05);background:linear-gradient(180deg,rgba(6,182,212,.07) 0%,transparent 100%);}
      .ws5-dot{width:8px;height:8px;border-radius:50%;background:#374151;flex-shrink:0;transition:all .3s;}
      .ws5-dot.on{background:#06b6d4;box-shadow:0 0 8px #06b6d4;animation:ws5p 2s infinite;}
      .ws5-dot.thinking{background:#f59e0b;box-shadow:0 0 8px #f59e0b;animation:ws5p .5s infinite;}
      @keyframes ws5p{0%,100%{opacity:1}50%{opacity:.4}}
      .ws5-title{font-size:12px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:#f0ecff;}
      .ws5-sub{font-size:9px;font-family:'DM Mono',monospace;color:#4b5563;}

      .ws5-stopbar{display:none;flex-shrink:0;padding:8px 12px;border-bottom:1px solid rgba(255,255,255,.06);background:rgba(0,0,0,.12);}
      .ws5-stopbar.show{display:block;}
      .ws5-stopbtn{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;padding:9px;background:rgba(239,68,68,.12);border:1px solid rgba(239,68,68,.35);border-radius:10px;cursor:pointer;font-family:'Syne',sans-serif;font-size:12px;font-weight:700;color:#fca5a5;letter-spacing:.04em;transition:all .2s;}
      .ws5-stopbtn:hover{background:rgba(239,68,68,.22);border-color:rgba(239,68,68,.5);}

      .ws5-status{display:none;flex-shrink:0;padding:5px 14px;font-family:'DM Mono',monospace;font-size:10px;color:#4b5563;border-bottom:1px solid rgba(255,255,255,.04);min-height:26px;align-items:center;}
      .ws5-status.show{display:flex;}
      .ws5-status.on{color:#06b6d4;}
      .ws5-status.thinking{color:#f59e0b;}
      .ws5-status.error{color:#f87171;}

      .ws5-cta{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:24px 16px;}
      .ws5-startbtn{display:flex;align-items:center;justify-content:center;gap:10px;width:100%;padding:16px;background:linear-gradient(135deg,rgba(6,182,212,.28),rgba(124,58,237,.22));border:1.5px solid rgba(6,182,212,.42);border-radius:14px;cursor:pointer;font-family:'Syne',sans-serif;font-size:16px;font-weight:800;color:#cffafe;letter-spacing:.02em;box-shadow:0 0 24px rgba(6,182,212,.12);transition:all .2s;}
      .ws5-startbtn:hover{background:linear-gradient(135deg,rgba(6,182,212,.42),rgba(124,58,237,.32));box-shadow:0 0 36px rgba(6,182,212,.25);transform:translateY(-1px);}
      .ws5-hint{font-size:10px;color:#374151;font-family:'DM Mono',monospace;text-align:center;line-height:1.7;max-width:210px;}

      .ws5-feed{display:none;flex:1;flex-direction:column;overflow-y:auto;scroll-behavior:smooth;scrollbar-width:thin;scrollbar-color:rgba(6,182,212,.25) transparent;}
      .ws5-feed.show{display:flex;}
      .ws5-feed::-webkit-scrollbar{width:3px;}
      .ws5-feed::-webkit-scrollbar-thumb{background:rgba(6,182,212,.25);border-radius:2px;}

      .ws5-msg{padding:9px 14px;font-size:12px;line-height:1.5;border-bottom:1px solid rgba(255,255,255,.025);flex-shrink:0;}
      .ws5-user{background:rgba(96,165,250,.07);border-left:3px solid rgba(96,165,250,.55);}
      .ws5-user .lbl{display:block;font-size:9px;font-family:'DM Mono',monospace;color:#60a5fa;letter-spacing:.1em;text-transform:uppercase;margin-bottom:3px;}
      .ws5-user .body{color:#bfdbfe;font-weight:600;}
      .ws5-response{background:rgba(6,182,212,.06);border-left:3px solid rgba(6,182,212,.45);}
      .ws5-response .lbl{display:block;font-size:9px;font-family:'DM Mono',monospace;color:#22d3ee;letter-spacing:.1em;text-transform:uppercase;margin-bottom:3px;}
      .ws5-response .body{color:#cffafe;}
      .ws5-action{font-family:'DM Mono',monospace;font-size:10px;color:#2d3748;padding:4px 14px;}
      .ws5-system{font-family:'DM Mono',monospace;font-size:10px;color:#374151;font-style:italic;text-align:center;}
      .ws5-error{font-family:'DM Mono',monospace;font-size:11px;color:#f87171;background:rgba(239,68,68,.05);border-left:3px solid rgba(239,68,68,.4);}
      .ws5-info{font-family:'DM Mono',monospace;font-size:10px;color:#a78bfa;}
      .ws5-confirm{font-size:12px;color:#fbbf24;background:rgba(251,191,36,.05);border-left:3px solid rgba(251,191,36,.4);}
      .ws5-warning{font-size:11px;color:#f59e0b;}

      .ws5-row{display:none;flex-shrink:0;gap:6px;padding:8px 10px;border-top:1px solid rgba(255,255,255,.05);align-items:center;}
      .ws5-row.show{display:flex;}
      .ws5-input{flex:1;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.09);border-radius:8px;padding:8px 10px;color:#f0ecff;font-family:'DM Mono',monospace;font-size:11px;outline:none;}
      .ws5-input::placeholder{color:#374151;}
      .ws5-input:focus{border-color:rgba(6,182,212,.35);}
      .ws5-sendbtn{background:rgba(6,182,212,.15);border:1px solid rgba(6,182,212,.3);border-radius:8px;padding:8px 12px;cursor:pointer;color:#22d3ee;font-size:13px;transition:all .2s;flex-shrink:0;}
      .ws5-sendbtn:hover{background:rgba(6,182,212,.25);}
      .ws5-clearbtn{background:transparent;border:1px solid rgba(255,255,255,.07);border-radius:8px;padding:8px 10px;cursor:pointer;color:#374151;font-size:11px;flex-shrink:0;transition:all .2s;}
      .ws5-clearbtn:hover{border-color:rgba(239,68,68,.3);color:#f87171;}

      #ws5-hover{position:fixed;z-index:2147483646;background:rgba(0,0,0,.88);color:#e2e8f0;font-size:12px;padding:5px 10px;border-radius:6px;pointer-events:none;max-width:280px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;border:1px solid rgba(255,255,255,.1);display:none;}
    `;
    ...
    root.innerHTML = `
      <div class="ws5-hdr">
        <div class="ws5-dot" id="ws5-dot"></div>
        <div>
          <div class="ws5-title">Web-Sight</div>
          <div class="ws5-sub">AI Browser Agent</div>
        </div>
      </div>
      <div class="ws5-stopbar" id="ws5-stopbar">
        <button class="ws5-stopbtn" id="ws5-stopbtn">⏹ &nbsp;Stop</button>
      </div>
      <div class="ws5-status" id="ws5-status"></div>
      <div class="ws5-cta" id="ws5-cta">
        <button class="ws5-startbtn" id="ws5-startbtn">
          <span style="font-size:22px">🌐</span> Start
        </button>
        <div class="ws5-hint" id="ws5-hint">Voice-controlled AI agent · controls your browser hands-free</div>
      </div>
      <div class="ws5-feed" id="ws5-feed"></div>
      <div class="ws5-row" id="ws5-row">
        <input class="ws5-input" id="ws5-input" placeholder="Or type a command…" />
        <button class="ws5-sendbtn" id="ws5-send">▶</button>
        <button class="ws5-clearbtn" id="ws5-clear">🗑</button>
      </div>
    `;
    paneEl.appendChild(root);
    outputEl = document.getElementById('ws5-feed');

    document.getElementById('ws5-startbtn').addEventListener('click', toggle);
    document.getElementById('ws5-stopbtn').addEventListener('click', toggle);
    document.getElementById('ws5-send').addEventListener('click', sendText);
    document.getElementById('ws5-input').addEventListener('keydown', e => { if (e.key === 'Enter') sendText(); });
    document.getElementById('ws5-clear').addEventListener('click', () => { clearHistory(); outputEl.innerHTML = ''; addMsg('system', 'Cleared.'); });

    if (!hoverOverlay) {
      hoverOverlay = document.createElement('div');
      hoverOverlay.id = 'ws5-hover';
    ...
    if (history.length > 0) {
      showActive();
      addMsg('system', `${history.length} messages from last session`);
  ...
  // ─── Layout ───────────────────────────────────────────────
  function showActive() {
    document.getElementById('ws5-cta').style.display = 'none';
    document.getElementById('ws5-stopbar').classList.add('show');
    document.getElementById('ws5-status').classList.add('show');
    document.getElementById('ws5-feed').classList.add('show');
    document.getElementById('ws5-row').classList.add('show');
  }
  function showIdle() {
    if (history.length === 0) {
      document.getElementById('ws5-cta').style.display = 'flex';
      document.getElementById('ws5-feed').classList.remove('show');
      document.getElementById('ws5-row').classList.remove('show');
    }
    document.getElementById('ws5-stopbar').classList.remove('show');
    document.getElementById('ws5-status').classList.remove('show');
  }
  function setStatus(msg, cls) {
    const el = document.getElementById('ws5-status');
    if (!el) return;
    el.textContent = msg;
    el.className = 'ws5-status show ' + (cls || '');
  }
  function setDot(state) {
    const el = document.getElementById('ws5-dot');
    if (el) el.className = 'ws5-dot' + (state ? ' ' + state : '');
  }
  ...
  async function startAgent() {
    ...
    try {
      ...
      if (!kr?.success) throw new Error('Could not get API key');
      apiKey = kr.apiKey;
      ...
      addMsg('error', err.message || 'Failed to start'); stopAgent(false);
    }
  }
  ...
  async function connectWS() {
    return new Promise((resolve, reject) => {
      ...
      ws.onopen = () => {
        ...
        ws.send(JSON.stringify({ type: 'session.update', session: { modalities: ['text','audio'], instructions: 'Transcription only. Output ONLY the English transcription of speech. No comments.', voice: 'alloy', input_audio_format: 'pcm16', output_audio_format: 'pcm16', input_audio_transcription: { model: 'whisper-1', language: 'en' }, turn_detection: { type: 'server_vad', threshold: 0.45, prefix_padding_ms: 300, silence_duration_ms: 800, create_response: false }, temperature: 0.6, max_response_output_tokens: 1 } }));
  ...
  function onWS(ev) {
    switch (ev.type) {
      case 'input_audio_buffer.speech_started': setStatus('Speech detected…', 'on'); break;
      ...
      case 'conversation.item.input_audio_transcription.completed':
        ...
        break;
      case 'response.audio.delta': if (ev.delta) playChunk(ev.delta); break;
      case 'response.done': if (!isRunning) { setDot('on'); setStatus('Listening… Speak your command!', 'on'); } break;
      case 'error':
        if (ev.error?.code === 'session_expired') { addMsg('system', 'Session expired, reconnecting…'); try { ws?.close(); } catch (e) {} }
        break;
    }
  }

  function playChunk(b64) {
    if (!pbCtx || pbCtx.state === 'closed') return;
    try {
      const bin = atob(b64); const bytes = new Uint8Array(bin.length);
      for (let i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i);
      const i16 = new Int16Array(bytes.buffer); const f32 = new Float32Array(i16.length);
      for (let i = 0; i < i16.length; i++) f32[i] = i16[i] / 32768;
      const buf = pbCtx.createBuffer(1, f32.length, 24000); buf.getChannelData(0).set(f32);
      const src = pbCtx.createBufferSource(); src.buffer = buf; src.connect(pbCtx.destination);
      const t = Math.max(pbCtx.currentTime, pbNext); src.start(t); pbNext = t + buf.duration;
    } catch (e) {}
  }

  // ─── Text input ───────────────────────────────────────────
  function sendText() {
    const inp = document.getElementById('ws5-input');
    if (!inp) return;
  ...
  // ─── Hover ────────────────────────────────────────────────
  function onHover(e) {
    if (!paneReady || !hoverOverlay) return;
    const t = e.target;
    if (!t || t.closest('#accessai-sidebar') || t.id === 'ws5-hover') return;
    const d = descEl(t);
    if (!d) { hoverOverlay.style.display = 'none'; return; }
    hoverOverlay.textContent = d; hoverOverlay.style.display = 'block';
    const r = t.getBoundingClientRect();
    hoverOverlay.style.left = Math.min(r.left, window.innerWidth - 300) + 'px';
    hoverOverlay.style.top = (r.bottom + 6) + 'px';
  }
  ...
  function addMsg(type, text) {
    ...
    const all = outputEl.querySelectorAll('.ws5-msg');
  ...
  window.addEventListener('accessai-mode-changed', e => {
    ...
    else { if (isActive) stopAgent(); if (hoverOverlay) hoverOverlay.style.display = 'none'; }
  });
